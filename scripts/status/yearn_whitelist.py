...
def main():
    ...
    for block in condition1Blocks:
        bBadgerAddreses = filter_zero(calculate_sett_balances(badger,"native.badger",bBadger,block)).keys()
        wbtcBadgerUniAddresses = filter_zero(calculate_sett_balances(badger,"native.uniBadgerWbtc",wbtcBadgerUni,block)).keys()
        wbtcBadgerSlpAddreses = filter_zero(calculate_sett_balances(badger,"native.sushiBadgerWbtc",wbtcBadgerSlp,block)).keys()
        combinedAddrs = set([*bBadgerAddreses,*wbtcBadgerUniAddresses,*wbtcBadgerSlpAddreses])
    ...
    for block in condition3Blocks:
        crvRenwbtcAddresses = filter_zero(calculate_sett_balances(badger,"native.renCrv",crvRenwbtc,block)).keys()
        crvRenWsBtcAddresses = filter_zero(calculate_sett_balances(badger,"native.sbtcCrv",crvRenWsBtc,block)).keys()
        tbtc_sbtcAddresses = filter_zero(calculate_sett_balances(badger,"native.tbtcCrv",tbtc_sbtcCrv,block)).keys()
        harvestRenBtcAddresses = filter_zero(calculate_sett_balances(badger,"harvest.renCrv",crvRenWbtc_harvest,block)).keys()
        wbtcWethSLPAddresses = filter_zero(calculate_sett_balances(badger,"native.sushiWbtcEth",wbtcWethSLP,block)).keys()
        combinedAddrs = set([*crvRenwbtcAddresses,*crvRenWsBtcAddresses,*tbtc_sbtcAddresses,*harvestRenBtcAddresses,*wbtcWethSLPAddresses])
    ...
    for block in condition4Blocks:
        uniDiggAddrs = filter_zero(calculate_sett_balances(badger,"native.uniDiggWbtc",wbtcDiggUni,block)).keys()
        sushiDiggAddrs =  filter_zero(calculate_sett_balances(badger,"native.sushiDiggWbtc",wbtcDiggSlp,block)).keys()
        bDiggAddrs = filter_zero(calculate_sett_balances(badger,"native.digg",bDigg,block)).keys()
    ...
    addrs = []
    uniDiggBalances_old = filter_zero(calculate_sett_balances(badger,"native.uniDiggWbtc",wbtcDiggUni,condition5Blocks[0]))
    uniDiggBalances_new = filter_zero(calculate_sett_balances(badger,"native.uniDiggWbtc",wbtcDiggUni,condition5Blocks[1]))
    for addr,bal in uniDiggBalances_new.items():
        old_bal = uniDiggBalances_old.get(addr,0)
        if bal > old_bal:
            addrs.append(addr)

    sushiDiggBalances_old =  filter_zero(calculate_sett_balances(badger,"native.sushiDiggWbtc",wbtcDiggSlp,condition5Blocks[0]))
    sushiDiggBalances_new =  filter_zero(calculate_sett_balances(badger,"native.sushiDiggWbtc",wbtcDiggSlp,condition5Blocks[1]))
    ...
    bDiggBalances_old = filter_zero(calculate_sett_balances(badger,"native.digg",bDigg,condition5Blocks[0]))
    bDiggBalances_new = filter_zero(calculate_sett_balances(badger,"native.digg",bDigg,condition5Blocks[1]))

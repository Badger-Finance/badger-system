...
def get_ethereum_data(block):
    bdiggHolders = calculate_sett_balances(badger, "native.digg", block)
    diggUlpHolders = calculate_sett_balances(badger, "native.uniDiggWbtc", block)
    diggSlpHolders = calculate_sett_balances(badger, "native.sushiDiggWbtc", block)
...
def json2csv(name, old, new):
    ...
    with open("bscData/{}.csv".format(name), "w") as csv_file:
        ...
        for addr in addrs:
            csv_writer.writerow(
                [addr, old.get(addr, 0) / 1e18, new.get(addr, 0) / 1e18]
            )

...
to_approve = ["native.hbtcCrv", "native.pbtcCrv", "native.obtcCrv", "native.bbtcCrv", "native.tricrypto"]
...
def main():
    ...
    for source_key in to_approve:
        ...
        for dest_key in destination_setts:
            destination = helper.contract_from_abi(badger.getSett(dest_key).address, "SettV3", SettV3.abi)
            console.print(f"Approve [green]{source}[/green] on Sett [yellow]{dest_key} ({destination.address})")

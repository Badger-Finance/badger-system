...
class BadgerRewardsManagerHelper():
    ...
    def approve_strategies_on_rewards_manager(self, strategy_keys):
        rm = self.helper.contract_from_abi(self.badger.badgerRewardsManager.address, "BadgerRewardsManager", BadgerRewardsManager.abi)
        for key in strategy_keys:
            strat = self.badger.getStrategy(key)
            console.print(f"Approving Strategy for keeping activities {key} {strat.address} on RewardsManager")
    ...
    def approve_setts_on_rewards_manager(self, sett_keys):
        ...
        for key in sett_keys:
            ...
            console.print(f"Granting role {APPROVED_SETT_ROLE} to Sett {key} {sett.address} on RewardsManager")

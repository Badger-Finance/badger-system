...
class ConvexTBtcMiniDeploy(SettMiniDeployBase):
    ...
    def post_vault_deploy_setup(self, deploy=True):
        if not deploy:
            return
        distribute_from_whales(self.deployer, 1, "tbtcCrv")
    ...
    def post_deploy_setup(self, deploy):
        ...
        newStrategy = badger_deploy["sett_system"]["strategies_registry"]["native.tbtcCrv"][

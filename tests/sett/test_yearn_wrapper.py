...
def test_yearn_simulation():
    badger_single_sett(yearnSettTestConfig[0], deploy=False)
    # Assign accounts
    deployer = accounts[0]
    ...
    mockToken.initialize(
        mockToken.contract_address, deployer.address, AddressZero, "YearnWBTC", "vyWBTC"
    )
    ...
    wrapper.initialize(
        mockToken.contract_address,
        yearnRegistry.contract_address,
        "BadgerYearnWBTC",
        "bvyWBTC",
        guardian.address,
    )

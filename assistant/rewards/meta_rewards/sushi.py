...
def calc_all_sushi_rewards(badger,startBlock,endBlock,nextCycle,retroactive):
    ...
    wbtcEthRewards = calc_sushi_rewards(
        ...
        allSushiEvents["wbtcEth"],"native.sushiWbtcEth",
        retroactive=retroactive,retroactiveStart=11537600
    )
    wbtcBadgerRewards = calc_sushi_rewards(
        badger,startBlock,endBlock,nextCycle,
        allSushiEvents["wbtcBadger"],"native.sushiBadgerWbtc",
        retroactive=retroactive,retroactiveStart=11539529
    )
    wbtcDiggRewards = calc_sushi_rewards(
        badger,startBlock,endBlock,nextCycle,
        allSushiEvents["wbtcDigg"],"native.sushiDiggWbtc",
        retroactive=retroactive,retroactiveStart=11676338
    )
    # Verify all rewards are correct (for extra safety)
    return combine_rewards([wbtcEthRewards,wbtcBadgerRewards,wbtcDiggRewards],nextCycle,badger.badgerTree)


def calc_sushi_rewards(badger,startBlock,endBlock,nextCycle,events,name,retroactive,retroactiveStart):
    return calc_rewards(
        badger,
        startBlock,
        endBlock,

...
def calc_geyser_rewards(badger, periodStartBlock, endBlock, cycle):
    ...
    for key, geyser in badger.geysers.items():
        ...
        geyserRewards = calc_geyser_stakes(
            key, geyser, periodStartBlock, endBlock)
        rewardsByGeyser[key] = geyserRewards
    return sum_rewards(rewardsByGeyser, cycle, badger.badgerTree)


def filter_events(e,startBlock,endBlock):
    return int(e["blockNumber"]) > startBlock and int(e["blockNumber"]) < endBlock


def calc_sett_sushi_rewards(name, badger, startBlock, endBlock, events, nextCycle):
    console.log(name)
    events = list(filter(lambda e: filter_events(e,startBlock,endBlock), events[name]))
    console.log(events)
    totalXSushi = sum([int(e["toBadgerTree"]) for e in events])
    rewards = RewardsList(nextCycle, badger.badgerTree)
    if len(events) > 0:
        rewards = process_sushi_events(
            badger, startBlock, endBlock, events, name, nextCycle
        )
    return rewards, totalXSushi


def calc_sushi_rewards(badger, startBlock, endBlock, nextCycle):
    ...
    totalxSushi = wbtcEthXSushi + badgerWbtcXSushi + diggWbtcXSushi + ibbtcWbtcXSushi

    finalRewards = combine_rewards(
        [wbtcEthRewards, wbtcBadgerRewards, wbtcDiggRewards, ibbtcWbtcRewards], nextCycle, badger.badgerTree)
    xSushiFromRewards = 0
    ...
    console.log("Difference {}".format(
        abs(totalxSushi/1e18 - xSushiFromRewards/1e18)))
    return finalRewards
...
def process_sushi_events(badger, startBlock, endBlock, events, name, nextCycle):
    ...
    for i in tqdm(range(len(events))):
        ...
        for user in user_state:
            rewards.increase_user_rewards(web3.toChecksumAddress(user.address), web3.toChecksumAddress(
            ...
            rewardsLogger.add_user_token(
                user.address, name, xSushiTokenAddress, xSushiUnit * user.shareSeconds)
...
def fetch_current_harvest_rewards(badger, startBlock, endBlock, nextCycle):
    ...
    for i in tqdm(range(len(unprocessedEvents))):
        ...
        user_state = calc_meta_farm_rewards(
            badger, "harvest.renCrv", start, end)
        ...
        for user in user_state:
            rewards.increase_user_rewards(user.address, web3.toChecksumAddress(
...
def calc_meta_farm_rewards(badger, name, startBlock, endBlock):
    console.log("Calculating rewards between {} and {}".format(
        startBlock, endBlock))
    startBlockTime = web3.eth.getBlock(startBlock)["timestamp"]
    endBlockTime = web3.eth.getBlock(endBlock)["timestamp"]
    if name == "experimental.sushiIBbtcWbtc":
        settId = "0x8a8ffec8f4a0c8c9585da95d9d97e8cd6de273de"
        geyserId = ""
    else:
    ...
    if len(settBalances) != 0:
        ...
        console.log("Geyser amount in sett Balance: {}".format(
            settBalances.get(geyserId,0)/1e18))
        if geyserId in settBalances:

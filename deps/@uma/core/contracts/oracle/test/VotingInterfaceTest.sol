...
abstract contract VotingInterfaceTesting is OracleInterface, VotingInterface, Testable {
    ...
    function rounds(uint256 roundId) public virtual view returns (Round memory);

    function getPriceRequestStatuses(VotingInterface.PendingRequest[] memory requests) public virtual view returns (RequestState[] memory);

    function getPendingPriceRequestsArray() external virtual view returns (bytes32[] memory);
}

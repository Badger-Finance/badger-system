...
import "deps/deps/@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "deps/deps/@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import "deps/deps/@openzeppelin/contracts/math/SafeMath.sol";
import "deps/deps/@openzeppelin/contracts/utils/Address.sol";
...
contract OptimisticOracle is OptimisticOracleInterface, Testable, Lockable {
    ...
    function setRefundOnDispute(
        ...
        require(getState(msg.sender, identifier, timestamp, ancillaryData) == State.Requested, "setRefundOnDispute: Requested");
    ...
    function setCustomLiveness(
        ...
        require(getState(msg.sender, identifier, timestamp, ancillaryData) == State.Requested, "setCustomLiveness: Requested");
    ...
    function proposePriceFor(
        ...
        require(getState(requester, identifier, timestamp, ancillaryData) == State.Requested, "proposePriceFor: Requested");
        Request storage request = _getRequest(requester, identifier, timestamp, ancillaryData);
        request.proposer = proposer;
        request.proposedPrice = proposedPrice;

        // If a custom liveness has been set, use it instead of the default.
        request.expirationTime = getCurrentTime().add(request.customLiveness != 0 ? request.customLiveness : defaultLiveness);
        ...
        // Callback.
        if (address(requester).isContract()) try OptimisticRequester(requester).priceProposed(identifier, timestamp, ancillaryData)  {} catch {}
    }
    ...
    function disputePriceFor(
        ...
        require(getState(requester, identifier, timestamp, ancillaryData) == State.Proposed, "disputePriceFor: Proposed");
        Request storage request = _getRequest(requester, identifier, timestamp, ancillaryData);
    ...
    function getRequest(
        ...
        uint256 timestamp,
        bytes memory ancillaryData
    ) public override view returns (Request memory) {
    ...
    function getState(
        ...
        uint256 timestamp,
        bytes memory ancillaryData
    ) public override view returns (State) {
        Request storage request = _getRequest(requester, identifier, timestamp, ancillaryData);
        ...
        return _getOracle().hasPrice(identifier, timestamp, _stampAncillaryData(ancillaryData, requester)) ? State.Resolved : State.Disputed;
    }
    ...
    function _settle(
        ...
        if (state == State.Expired) {
            ...
            request.resolvedPrice = _getOracle().getPrice(identifier, timestamp, _stampAncillaryData(ancillaryData, requester));
        ...
        // Event.
        emit Settle(requester, request.proposer, request.disputer, identifier, timestamp, ancillaryData, request.resolvedPrice, payout);

        // Callback.
        if (address(requester).isContract())
            try OptimisticRequester(requester).priceSettled(identifier, timestamp, ancillaryData, request.resolvedPrice)  {} catch {}
    }

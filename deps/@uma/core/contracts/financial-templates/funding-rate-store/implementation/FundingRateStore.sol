...
contract FundingRateStore is FundingRateStoreInterface, Testable, Lockable {
        ...
        returns (FixedPoint.Unsigned memory rewardRate)
    {
        ...
        rewardRate = _calculateProposalRewardPct(perpetual, fundingRateRecord.proposeTime, getCurrentTime(), rate, fundingRateRecord.rate);
    }
     ...
     * @param rate Proposed rate.
     */
    function propose(address perpetual, FixedPoint.Signed memory rate) external nonReentrant() publishAndWithdrawProposal(perpetual) {
        ...
        require(_getFundingRateDispute(perpetual, currentTime).proposal.proposer == address(0x0), "Proposal pending dispute");
        ...
        emit ProposedRate(perpetual, rate.rawValue, currentTime, msg.sender, rewardRate.rawValue, proposalBond.rawValue, finalFeeBond.rawValue);
     ...
     */
    function settleDispute(address perpetual, uint256 proposalTime) external nonReentrant() publishAndWithdrawProposal(perpetual) {
        ...
        // time will be 0.
        FixedPoint.Signed memory settlementRate = _getOraclePrice(PerpetualInterface(perpetual).getFundingRateIdentifier(), proposalTime);
        ...
        address proposer = disputeSucceeded ? fundingRateDispute.proposal.disputer : fundingRateDispute.proposal.proposer;
        ...
        if (disputeSucceeded) {
            ...
            FixedPoint.Unsigned memory disputerRebate = fundingRateDispute.proposal.proposalBond.add(fundingRateDispute.proposal.finalFee);
            FixedPoint.Unsigned memory disputerReward = fundingRateDispute.proposal.proposalBond;

            collateralCurrency.safeTransfer(fundingRateDispute.proposal.disputer, disputerReward.add(disputerRebate).rawValue);
        } else {
            // If dispute fails:
    ...
    function _publishRateAndWithdrawRewards(address perpetual) internal {
        ...
        IERC20 collateralCurrency = IERC20(PerpetualInterface(perpetual).getCollateralCurrency());
        FixedPoint.Unsigned memory amountToPay = fundingRateRecord.proposal.finalFee.add(fundingRateRecord.proposal.proposalBond);
        try PerpetualInterface(perpetual).withdrawFundingRateFees(reward) returns (FixedPoint.Unsigned memory rewardWithdrawn) {
    ...
    function _getOraclePrice(bytes32 identifier, uint256 requestedTime) internal view returns (FixedPoint.Signed memory) {

...
contract DepositBox is FeePayer, ContractCreator {
    ...
    event RequestWithdrawalCanceled(address indexed user, uint256 indexed collateralAmount, uint256 requestPassTimestamp);
    ...
    ) public ContractCreator(_finderAddress) FeePayer(_collateralAddress, _finderAddress, _timerAddress) nonReentrant() {
    ...
    function executeWithdrawal() external isInitialized() fees() nonReentrant() returns (FixedPoint.Unsigned memory amountWithdrawn) {
        ...
        FixedPoint.Unsigned memory denominatedAmountToWithdraw = depositBoxData.withdrawalRequestAmount.div(exchangeRate);
        ...
        emit RequestWithdrawalExecuted(msg.sender, amountWithdrawn.rawValue, exchangeRate.rawValue, depositBoxData.requestPassTimestamp);
    ...
    function cancelWithdrawal() external isInitialized() nonReentrant() {
        ...
        emit RequestWithdrawalCanceled(msg.sender, depositBoxData.withdrawalRequestAmount.rawValue, depositBoxData.requestPassTimestamp);
    ...
    function _incrementCollateralBalances(DepositBoxData storage depositBoxData, FixedPoint.Unsigned memory collateralAmount)
        internal

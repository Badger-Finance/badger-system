...
contract FinancialProductLibraryTest is FinancialProductLibrary {
    ...
    function transformCollateralRequirement(FixedPoint.Unsigned memory price, FixedPoint.Unsigned memory collateralRequirement)
        public
        override
        view
        returns (FixedPoint.Unsigned memory)
    ...
    function transformPriceIdentifier(bytes32 priceIdentifier, uint256 requestTime) public override view returns (bytes32) {
        require(!shouldRevert, "set to always reverts");

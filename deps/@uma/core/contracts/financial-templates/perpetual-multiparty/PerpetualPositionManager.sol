...
contract PerpetualPositionManager is FundingRatePayer, FundingRateApplier {
    ...
    event SettleEmergencyShutdown(address indexed caller, uint256 indexed collateralReturned, uint256 indexed tokensBurned);
        ...
        returns (FixedPoint.Unsigned memory amountWithdrawn)
    {
        PositionData storage positionData = _getPositionData(msg.sender);
        require(
            positionData.withdrawalRequestPassTimestamp != 0 && positionData.withdrawalRequestPassTimestamp <= getCurrentTime(),
        ...
        returns (FixedPoint.Unsigned memory amountWithdrawn)
    {
        ...
        FixedPoint.Unsigned memory totalRedeemableCollateral = _getFundingRateAppliedTokenDebt(tokensToRedeem).mul(emergencyShutdownPrice);
        ...
        if (_getFeeAdjustedCollateral(positionData.rawCollateral).isGreaterThan(0)) {
            ...
            FixedPoint.Unsigned memory tokenDebtValueInCollateral = _getFundingRateAppliedTokenDebt(positionData.tokensOutstanding).mul(
            ...
            FixedPoint.Unsigned memory positionRedeemableCollateral = tokenDebtValueInCollateral.isLessThan(positionCollateral)
    ...
    function _reduceSponsorPosition(
        ...
        require(positionData.tokensOutstanding.isGreaterThanOrEqual(minSponsorTokens), "Below minimum sponsor position");

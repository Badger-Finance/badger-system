...
contract OperatorForwarder {
    ...
    constructor(address link) {
        ...
        address[] memory authorizedSenders = OperatorInterface(msg.sender)
            .getAuthorizedSenders();
        authorizedSender2 = (authorizedSenders.length > 0)
            ? authorizedSenders[0]
            : address(0);
        authorizedSender3 = (authorizedSenders.length > 1)
            ? authorizedSenders[1]
            : address(0);
    }

...
contract MeanAggregator is ServiceAgreementDecoder {
    ...
    {
        if (
            reportingOrder[_requestId][_oracle] != 0 ||
        ...
        uint256 newAverage = average[_requestId] +
            oDividend +
            (newRemainder / oracles[_sAId].length);
    ...
    function calculatePayments(bytes32 _sAId, bytes32 _requestId)
        private
        returns (int256[] memory paymentAmounts)
    {
        ...
        for (uint256 oIdx = 0; oIdx < oracles[_sAId].length; oIdx++) {
            // Linearly-decaying payout determined by each oracle's reportingIdx
                ...
                uint256 reportingIdx
             = reportingOrder[_requestId][oracles[_sAId][oIdx]] - 1;
            paymentAmounts[oIdx] = int256(
                2 *
                    (totalPayment / numOracles) -
                    ((totalPayment * ((2 * reportingIdx) + 1)) /
                        (numOracles**2))
            );

...
contract MaliciousRequester is MaliciousChainlinkClient {
    ...
    function request(
        ...
        Chainlink.Request memory req = buildChainlinkRequest(
            _id,
            _target,
            bytes4(keccak256(_callbackFunc))
        );
    ...
    function maliciousTargetConsumer(address _target)
        public
        returns (bytes32 requestId)
    {
        Chainlink.Request memory req = buildChainlinkRequest(
            'specId',
            _target,
            bytes4(keccak256('fulfill(bytes32,bytes32)'))
        );
        requestId = chainlinkTargetRequest(_target, req, ORACLE_PAYMENT);
    }

    function maliciousRequestCancel(bytes32 _id, bytes memory _callbackFunc)
        public
    {
        ChainlinkRequestInterface _oracle = ChainlinkRequestInterface(

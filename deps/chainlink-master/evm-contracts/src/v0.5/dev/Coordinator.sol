    ...
    OracleSignaturesDecoder
{
    ...
    function initiateServiceAgreement(
        ...
        ServiceAgreement memory _agreement = decodeServiceAgreement(
            _serviceAgreementData
        );
        OracleSignatures memory _signatures = decodeOracleSignatures(
        ...
        // solhint-disable-next-line not-rely-on-time
        require(
            _agreement.endAt > block.timestamp,
            'ServiceAgreement must end in the future'
        );
        require(
            serviceAgreements[serviceAgreementID].endAt == 0,
        ...
        (bool success, bytes memory message) = abi.decode(
            response,
            (bool, bytes)
        );
    ...
    function registerOracleSignatures(
        ...
        for (uint256 i = 0; i < _oracles.length; i++) {
            ...
            require(
                _oracles[i] == signer,
                'Invalid oracle signature specified in SA'
            );
        ...
        returns (bool)
    {
        ...
        ) = abi.decode(aggResponse, (bool, bool, bytes, int256[])); // solhint-disable-line
        require(aggSuccess, string(response));
        if (aggComplete) {
            require(
                paymentAmounts.length == sA.oracles.length,
                'wrong paymentAmounts.length'
            );
            for (uint256 oIdx = 0; oIdx < sA.oracles.length; oIdx++) {
                // pay oracles
                withdrawableTokens[sA.oracles[oIdx]] = uint256(
                    int256(withdrawableTokens[sA.oracles[oIdx]]) +
                        paymentAmounts[oIdx]
                );
            } // solhint-disable-next-line avoid-low-level-calls
    ...
    function onTokenTransfer(
        ...
        uint256 _amount,
        bytes memory _data
    ) public onlyLINK permittedFunctionsForLINK {
        assembly {
    ...
    modifier isValidRequest(bytes32 _requestId) {
        require(
            callbacks[_requestId].addr != address(0),
            'Must have a valid requestId'
        );
        require(
            allowedOracles[callbacks[_requestId].sAId][msg.sender],
            'Oracle not recognized on service agreement'
        );
        _;
    }
    ...
    modifier sufficientLINK(uint256 _amount, bytes32 _sAId) {
        require(
            _amount >= serviceAgreements[_sAId].payment,
    ...
    modifier permittedFunctionsForLINK() {
        ...
        require(
            funcSelector[0] == this.oracleRequest.selector ||
                funcSelector[0] == this.depositFunds.selector,
    ...
    modifier checkServiceAgreementPresence(bytes32 _sAId) {
        require(
            uint256(serviceAgreements[_sAId].requestDigest) != 0,

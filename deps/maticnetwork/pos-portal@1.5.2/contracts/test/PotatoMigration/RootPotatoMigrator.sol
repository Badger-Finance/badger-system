...
contract RootPotatoMigrator {
    ...
    function plantOnChildFarm(uint256 amount) external {
        potato.transferFrom(msg.sender, address(this), amount);

        potato.approve(erc20Predicate, amount);

        rootChainManager.depositFor(childPotatoMigrator, address(potato), abi.encode(amount));

        stateSender.syncState(childPotatoMigrator, abi.encode(msg.sender, amount));
    }
}

...
contract MintableERC1155Predicate is ITokenPredicate, ERC1155Receiver, AccessControlMixin, Initializable {
    ...
    bytes32 public constant TRANSFER_SINGLE_EVENT_SIG = keccak256("TransferSingle(address,address,address,uint256,uint256)");
    bytes32 public constant TRANSFER_BATCH_EVENT_SIG = keccak256("TransferBatch(address,address,address,uint256[],uint256[])");
    ...
    function lockTokens(
        ...
        emit LockedBatchMintableERC1155(depositor, depositReceiver, rootToken, ids, amounts);
        IMintableERC1155(rootToken).safeBatchTransferFrom(depositor, address(this), ids, amounts, data);
    }
    ...
    function makeArrayWithValue(uint256 val, uint256 size) internal pure returns (uint256[] memory) {
    ...
    {
        require(tokenBalances.length == amountsToBeExited.length, "MintableERC1155Predicate: Array length mismatch found");
    ...
    function exitTokens(
        ...
        if (bytes32(logTopicRLPList[0].toUint()) == TRANSFER_SINGLE_EVENT_SIG) {
            // topic0 is event sig
            (uint256 id, uint256 amount) = abi.decode(logData, (uint256, uint256));
            ...
            if (tokenBalance < amount) {
                ...
                token.mintBatch(address(this), makeArrayWithValue(id, 1), makeArrayWithValue(amount - tokenBalance, 1), bytes(""));
            }

            token.safeTransferFrom(address(this), withdrawer, id, amount, bytes(""));
        } else if (bytes32(logTopicRLPList[0].toUint()) == TRANSFER_BATCH_EVENT_SIG) {
            (uint256[] memory ids, uint256[] memory amounts) = abi.decode(logData, (uint256[], uint256[]));
            ...
            token.mintBatch(
                address(this),
                ids,
                calculateAmountsToBeMinted(token.balanceOfBatch(makeArrayWithAddress(address(this), ids.length), ids), amounts),
                bytes("")
            );

            IMintableERC1155(rootToken).safeBatchTransferFrom(address(this), withdrawer, ids, amounts, bytes(""));
        } else {
